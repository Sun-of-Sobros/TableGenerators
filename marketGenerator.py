...
Poisons = [Contact, Injury, Inhaled, Injested]
...
def getPoisons(requested_type, num_poisons=1):
    ...
    for i in range(num_poisons):
        ...
        while True:
            ...
            # Choose a random poison type if not specified, otherwise use the requested type
            if requested_type not in typesOfPoisons:
                poison_type = random.choice(typesOfPoisons)
            else:
                poison_type = requested_type
            
            # Get a random poison of the chosen type
            poison_list = Poisons[poison_type]
            randNum = randint(0, len(poison_list) - 1)
            newPoison = poison_list[randNum]

            # Check if this new poison is already added to the list to avoid duplicates
            if newPoison not in poisons:
                poisons.append(newPoison)  # Directly append the dictionary representing the poison
                break
            elif attempts > 100:
                # Prevent infinite loops by giving up after 100 attempts to find a unique item
                print("Exceeded maximum attempts for this item", attempts)
                break
